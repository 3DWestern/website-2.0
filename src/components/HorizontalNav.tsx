...
export function HorizontalNav({
  ...
  return (
    <>
      ...
      >
        <div
          className={`${navStyles.bg} ${navStyles.backdrop} border ${navStyles.border} rounded-full ${navStyles.shadow} ${isHeroOnly ? "px-6 lg:px-10" : "px-12"} py-5`}
        >
      ...
      <div className="lg:hidden">
        ...
        <div
          className={`${isHeroOnly ? "absolute" : "fixed"} top-4 ${isHeroOnly ? "left-2 right-2 max-w-[calc(100%-1rem)] mx-auto" : "left-4 right-4"} z-50 ${navStyles.bg} ${navStyles.backdrop} border ${navStyles.border} rounded-full ${navStyles.shadow}`}
        >
          <div className="flex items-center justify-between px-4 py-3">
            <Link
              href="/"
              className="w-10 h-10 relative bg-white rounded-full p-1.5"
            >
              <Image
                src="/logo.png"
                alt="3D Western Logo"
