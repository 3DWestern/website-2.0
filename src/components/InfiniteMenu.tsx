...
/**
 * Renders many individual disc meshes on the surface of an invisible sphere 
 * 
 * 
 */

/**
 * The vertex shader: uses aInstanceMatrix to position a generic Disc geometry at many locations of the 
 * sphere.
 * stretchDir and strength computed based on uRotationAxisVelocity 
 * 
 * rotationVelocity increases when the user drags the globe fast 
 * 
 */
...
void main() {
    ...
    worldPosition.xyz = radius * normalize(worldPosition.xyz); // makes sure it stays to the surface of the sphere
...
/**
 * Loads the images as a single large grid for the sphere's discs.
 * 
 */
...
// generate the isoahedron and form a sphere with it 
...
class ArcballControl {
	...
	public update(deltaTime: number, targetFrameDuration = 16): void {
		...
		// attempt 1: new autoRotation quat 
		...
		if (this.isPointerDown) {
			...
			if (this.autoRotate) {
				//make a constant rotation based on speed and time 
				// angle = speed * timescale * arbitrary factor (for user feel)
				const angle = this.autoRotateSpeed * timeScale * 0.01;
				quat.setAxisAngle(autoRotationQuat, this.autoRotateAxis, angle);
		...
		// apply auto rotation (if active) to the thing 
		if (!this.isPointerDown && this.autoRotate) {
			quat.multiply(this.orientation, autoRotationQuat, this.orientation);
		}

		// apply user drag / snapping 
